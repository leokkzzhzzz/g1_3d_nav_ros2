...
void VirtualWallTool::autoLoadVirtualWalls()
{
  ...
  if (nh.getParam("/map_server/map_file", map_file) || nh.getParam("/rviz/map_file", map_file))
  {
    ...
    {
      test_file.close();
      ROS_INFO("Found matching virtual wall file, loading: %s", vw_file.c_str());
      
      // 延迟加载以确保RViz完全初始化
      QTimer::singleShot(1000, [this, vw_file]() {
        loadVirtualWalls(vw_file);
        setStatus("Virtual walls loaded: " + QString::fromStdString(vw_file));
      });
    }
    else
    {
      ROS_INFO("No virtual wall file for current map: %s, publishing empty", vw_file.c_str());
      setStatus("当前地图无Virtual walls，显示为空");
    }
  }
  else
  {
    ROS_INFO("No map file param, publishing empty virtual walls");
    setStatus("No map loaded — virtual walls empty");
  }
}

void VirtualWallTool::activate()
{
  setStatus("Virtual Wall Tool — LMB: click two points to create wall, RMB: cancel");
  updateMarkers();
}
...
int VirtualWallTool::processMouseEvent(rviz::ViewportMouseEvent& event)
{
  ...
  else if (event.rightDown() && drawing_wall_)
  {
    ...
    drawing_wall_ = false;
    updateMarkers();
    setStatus("Cancelled current virtual wall");
  }
...
{
  // 新增方法：根据地图文件路径加载对应的Virtual walls
  virtual_walls_.clear();
  current_wall_points_.clear();
  drawing_wall_ = false;
  ...
  {
    test_file.close();
    ROS_INFO("Loading virtual wall file: %s", vw_file.c_str());
    loadVirtualWalls(vw_file);
  }
  else
  {
    ROS_INFO("Map %s has no virtual wall file, showing empty", map_file_path.c_str());
