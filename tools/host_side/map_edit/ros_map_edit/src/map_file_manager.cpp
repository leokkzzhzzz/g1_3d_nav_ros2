...
bool MapFileManager::saveMapFiles(const std::string& yaml_filename)
{
  ...
  if (!map_msg)
  {
    ...
    if (!map_msg)
    {
      last_error_ = "Cannot get map data — make sure the map topic is being published";
      return false;
    }
  }
...
bool MapFileManager::loadYAML(const std::string& filename, nav_msgs::OccupancyGrid& map)
{
  try
  {
    ROS_INFO("Parsing YAML: %s", filename.c_str());
    ...
    ROS_INFO("YAML parsed — resolution %.3f, origin [%.2f, %.2f], occupied_thresh %.3f, free_thresh %.3f, negate %s", 
    ...
    ROS_INFO("Image file from YAML: %s", image_file.c_str());
    ...
    ROS_INFO("Resolved PGM path: %s", pgm_filename.c_str());
    
    // 检查PGM文件是否存在
    std::ifstream test_file(pgm_filename);
    if (!test_file.good())
    {
      last_error_ = "PGM not found: " + pgm_filename;
      ROS_ERROR("PGM not found: %s", pgm_filename.c_str());
      ...
      for (const auto& search_path : search_paths)
      {
        ...
        if (search_file.good())
        {
          ...
          ROS_INFO("Found PGM at alternate path: %s", pgm_filename.c_str());
          break;
        }
      }
    }
    ...
    ROS_INFO("Trying to load PGM: %s", pgm_filename.c_str());
    bool success = loadPGMWithParams(pgm_filename, map, occupied_thresh, free_thresh, negate);
    
    if (success)
    {
      ROS_INFO("PGM loaded: %dx%d px", map.info.width, map.info.height);
    }
    else
    {
      ROS_ERROR("PGM load failed: %s", last_error_.c_str());
    }
