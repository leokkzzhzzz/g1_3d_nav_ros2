...
void MapEraserTool::onInitialize()
{
  ...
  // Setup only brush size property - 改为 px单位
...
{
  setStatus("B/W Eraser — LMB: black (obstacle), RMB: white (free), drag to paint");
  updateProperties();
}
...
int MapEraserTool::processKeyEvent(QKeyEvent* event, rviz::RenderPanel* panel)
{
  if (event->modifiers() & Qt::ControlModifier)
  {
    if (event->key() == Qt::Key_Plus || event->key() == Qt::Key_Equal)
    {
      ...
      updateProperties();
      setStatus("Brush size up: " + QString::number(brush_size_property_->getFloat()) + "  px");
      return Render;
    }
    else if (event->key() == Qt::Key_Minus || event->key() == Qt::Key_Underscore)
    {
      ...
      updateProperties();
      setStatus("Brush size down: " + QString::number(brush_size_property_->getFloat()) + "  px");
      return Render;
    }
  }
...
void MapEraserTool::updateProperties()
{
  ...
  QString status_msg = "B/W Eraser — brush: " + QString::number(size) + "x" + QString::number(size) + " px, LMB: black, RMB: white";
...
void MapEraserTool::publishModifiedMap()
{
  ...
  // 记录发布信息 - 更新为 px单位
  int size = static_cast<int>(brush_size_);
  setStatus("Map edited and published — brush: " + QString::number(size) + "x" + QString::number(size) + " px");
}
