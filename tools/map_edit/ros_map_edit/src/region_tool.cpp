...
void RegionTool::autoLoadRegions()
{
  ...
  if (nh.getParam("/map_server/map_file", map_file) || nh.getParam("/rviz/map_file", map_file))
  {
    ...
    {
      test_file.close();
      ROS_INFO("Found matching region file, loading: %s", region_file.c_str());
      
      // 延迟加载以确保RViz完全初始化
      QTimer::singleShot(1000, [this, region_file]() {
        loadRegions(region_file);
        setStatus("Regions loaded: " + QString::fromStdString(region_file));
      });
    }
    else
    {
      ROS_INFO("No region file for current map: %s, publishing empty", region_file.c_str());
      setStatus("No region file for current map — showing empty");
    }
  }
  else
  {
    ROS_INFO("No map file param, publishing empty regions");
    setStatus("No map loaded — regions empty");
  }
}

void RegionTool::loadRegionsForMap(const std::string& map_file_path)
{
  // 新增方法：根据地图文件路径加载对应的Regions
  regions_.clear();
  current_region_points_.clear();
  drawing_region_ = false;
  ...
  {
    test_file.close();
    ROS_INFO("Loading region file: %s", region_file.c_str());
    loadRegions(region_file);
  }
  else
  {
    ROS_INFO("Map %s has no region file, showing empty", map_file_path.c_str());
